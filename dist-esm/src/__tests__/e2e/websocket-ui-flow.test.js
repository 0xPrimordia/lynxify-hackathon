...
jest.mock('../../app/components/RebalanceProposalModal', () => {
    return function MockRebalanceProposalModal({ isOpen, onClose, onSubmit }) {
        return isOpen ? (data - testid) = "rebalance-modal" >
            data - testid : ;
        "submit-proposal";
        onClick = {}();
        onSubmit({ 'BTC': 0.5, 'ETH': 0.3, 'SOL': 0.2 });
    } >
        Submit;
    Proposal
        < /button>
...
jest.mock('../../app/components/AgentStatus', () => {
    return function MockAgentStatus({ status }) {
        return (data - testid) = "agent-status" >
            Status;
        {
...
jest.mock('../../app/components/HCSMessageFeed', () => {
    return function MockHCSMessageFeed({ messages }) {
        return (data - testid) = "message-feed" >
            { messages, : .map((msg, idx) => key = { idx }, data - testid, {} `message-${idx}`) } >
            { msg, : .type };
        {
            msg.id;
        }
...
jest.mock('../../app/components/IndexComposition', () => {
    return function MockIndexComposition({ weights }) {
        return (data - testid) = "index-composition" >
            { Object, : .entries(weights).map(([token, weight]) => key = { token }, data - testid, {} `weight-${token}`) } >
            { token };
        {
            weight;
        }
...
describe('End-to-End WebSocket UI Flow', () => {
    ...
    beforeEach(async () => {
        ...
        mockWebSocketClient.on('_sent', (data) => {
            try {
        ...
        render(status, { WebSocketContext, : .status } /  >
            weights, { WebSocketContext, : .weights } /  >
            messages, { WebSocketContext, : .messages } /  >
...
test('Complete rebalance proposal, approval, and execution flow', async () => {
    ...
    act(() => {
        render(isOpen, { true:  }, onClose = { jest, : .fn() }, onSubmit = {}(weights), {
...
act(() => {
    ...
    render(messages, { WebSocketContext, : .messages } /  >
    );
});
...
test('Risk alert propagation from agent to UI', async () => {
    ...
    expect(eventBus.emitEvent).toHaveBeenCalledWith(EventType.INDEX_RISK_ALERT, expect.objectContaining({
        severity: 'high',
