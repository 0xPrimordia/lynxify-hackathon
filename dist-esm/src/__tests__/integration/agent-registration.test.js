...
describe('Agent Registration Integration', () => {
    ...
    beforeEach(() => {
        ...
        fs.writeFile.mockImplementation((path, data) => Promise.resolve());
    });
    test('should register a new agent with Moonscape', async () => {
        ...
        const result = await agentRegistry.registerAgent(operatorId, operatorKey, registryUrl, registryTopic);
        // Verify results
        ...
        expect(fs.writeFile).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('accountId'));
    });
    test('should detect previously registered agent', async () => {
        ...
        // Check if registered
        const isRegistered = await agentRegistry.isAlreadyRegistered('0.0.4340026', '0.0.12345');
        // Verify results
    ...
    test('should retrieve stored registration information', async () => {
        ...
        fs.readFile.mockResolvedValue(JSON.stringify(storedData));
        // Get stored info
    ...
    test('should handle registration failure', async () => {
        ...
        const result = await agentRegistry.registerAgent('0.0.4340026', 'mock-private-key', 'https://moonscape.tech', '0.0.12345');
