...
class MockUnifiedWebSocketServer {
    constructor(config) {
        ...
        this.stop = mockStop;
        this.broadcastToAll = mockBroadcast;
...
describe('Client-Agent Integration Flows', () => {
    ...
    let webSocketServer; // Use any type for our mock
    ...
    test('Client-Initiated Price Update Flow', async () => {
        ...
        expect(eventBus.emitEvent).toHaveBeenCalledWith(EventType.INDEX_PRICE_UPDATED, expect.objectContaining({
            tokenId: '0.0.1001',
            price: 65000
        }));
    });
    ...
    test('Client-Initiated Governance Flow', async () => {
        ...
        expect(eventBus.emitEvent).toHaveBeenCalledWith(EventType.GOVERNANCE_PROPOSAL_CREATED, expect.objectContaining({
            title: 'Change Risk Threshold'
        }));
        // Verify client received confirmation
        expect(mockWebSocketClient.send).toHaveBeenCalledWith(expect.stringContaining('GovernanceProposalCreated'));
    });
    /**
    ...
    test('Risk Alert Propagation to Client Flow', async () => {
        ...
        expect(mockWebSocketClient.send).toHaveBeenCalledWith(expect.stringContaining('RiskAlert'));
        // Verify risk alert details
        const alertCall = mockWebSocketClient.send.mock.calls.find(call => call[0] && JSON.parse(call[0]).type === 'RiskAlert');
        if (alertCall) {
