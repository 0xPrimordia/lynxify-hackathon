...
import { EventBus } from '../../app/utils/event-emitter';
...
jest.mock('../../app/utils/event-emitter', () => {
    ...
    mockEventBus.emitEvent = jest.fn().mockImplementation((eventType, eventData) => {
        ...
        if (onceHandlers.length > 0) {
            onceHandlers.forEach((handler) => handler(eventData));
            onceListeners.set(eventType, []);
        }
        return true; // Return true to match the real emitEvent's return type
    });
    mockEventBus.onEvent = jest.fn().mockImplementation((eventType, handler) => {
        // console.log(`Adding handler for event: ${eventType}`);
        if (!listeners.has(eventType)) {
            listeners.set(eventType, []);
        }
        listeners.get(eventType).push(handler);
        return mockEventBus; // Return this for chaining
    });
    mockEventBus.offEvent = jest.fn().mockImplementation((eventType, handler) => {
        if (listeners.has(eventType)) {
            const handlers = listeners.get(eventType);
    ...
    mockEventBus.onceEvent = jest.fn().mockImplementation((eventType, handler) => {
    ...
    mockEventBus.handleMessageReceived = (messageData) => {
...
const createMockHederaService = (failureMode) => {
    const mock = {
        ...
        shutdown: jest.fn().mockResolvedValue(undefined),
        subscribeToTopic: jest.fn().mockImplementation((topicId) => {
        ...
        publishMessage: jest.fn().mockImplementation((topicId, message) => {
...
const createMockTokenService = (failureMode) => {
    const mockTokenIds = {
        'BTC': '0.0.1001',
        'ETH': '0.0.1002',
        'SOL': '0.0.1003',
