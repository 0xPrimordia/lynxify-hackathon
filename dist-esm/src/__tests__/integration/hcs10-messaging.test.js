...
describe('HCS-10 Messaging Integration', () => {
    ...
    test('should format and send message in HCS-10 format to Moonscape', async () => {
        ...
        // Send message
        const result = await hcsMessaging.sendToMoonscape(outboundTopicId, inboundTopicId, operatorId, message);
        // Verify results
        expect(result).toBe(true);
        // Verify HederaService was called with properly formatted HCS-10 message
        expect(hederaService.publishHCSMessage).toHaveBeenCalledWith(outboundTopicId, expect.objectContaining({
    ...
    test('should send agent status updates in HCS-10 format', async () => {
        ...
        // Send status update
        const result = await hcsMessaging.sendAgentStatus(outboundTopicId, inboundTopicId, operatorId, pendingProposals, executedProposals);
        // Verify results
        expect(result).toBe(true);
        ...
        const callArgs = hederaService.publishHCSMessage.mock.calls[0];
    ...
    test('should handle errors when sending fails', async () => {
        ...
        hederaService.publishHCSMessage.mockRejectedValue(new Error('Network error'));
        // Setup test data
        const topicId = '0.0.12345';
        const message = {
            id: 'test-message',
