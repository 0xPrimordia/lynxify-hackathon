...
describe('Unified Agent Integration', () => {
    ...
    beforeEach(() => {
        ...
        hcs10Service = new HCS10ProtocolService(hederaService, testConfig.hcs10Config);
        tokenizedIndexService = new TokenizedIndexService(hederaService, testConfig.indexConfig);
        // Store the services in the agent instance for testing
        lynxifyAgent.hederaService = hederaService;
        lynxifyAgent.hcs10Service = hcs10Service;
        lynxifyAgent.indexService = tokenizedIndexService;
        // Set initialized flag to true to avoid calling the real initialize
        hcs10Service.initialized = true;
        tokenizedIndexService.initialized = true;
        ...
        hcs10Service.shutdown = jest.fn().mockResolvedValue(undefined);
        tokenizedIndexService.shutdown = jest.fn().mockResolvedValue(undefined);
    });
    afterEach(() => {
    ...
    test('complete agent initialization flow', async () => {
        ...
        expect(eventBus.emitEvent).toHaveBeenCalledWith(EventType.SYSTEM_INITIALIZED, expect.anything());
    });
    test('agent should handle HCS10 protocol messages', async () => {
        ...
        expect(eventBus.emitEvent).toHaveBeenCalledWith(EventType.MESSAGE_RECEIVED, expect.objectContaining({
    ...
    test('agent shutdown flow', async () => {
        ...
        expect(eventBus.emitEvent).toHaveBeenCalledWith(EventType.SYSTEM_SHUTDOWN, undefined);
    });
    test('HCS10 protocol service should register agent', async () => {
        // Setup mocks for this test with specific implementations
        jest.spyOn(hederaService, 'initialize').mockResolvedValue(undefined);
        jest.spyOn(hederaService, 'createTopic').mockImplementation(async (memo) => {
        ...
        const hcs10Service = new HCS10ProtocolService(hederaService, testHCS10Config);
        ...
        expect(hederaService.sendMessage).toHaveBeenCalledWith(testConfig.hcs10Config.registryTopicId, expect.stringContaining('"type":"AgentInfo"'));
    });
});
