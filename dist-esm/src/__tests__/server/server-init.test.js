...
describe('Server Initialization', () => {
    ...
    beforeEach(() => {
        ...
        getOptionalEnv.mockReturnValue('mock-value');
        agentRegistry.isAlreadyRegistered.mockResolvedValue(true);
        agentRegistry.getStoredRegistrationInfo.mockResolvedValue({
            accountId: '0.0.123',
    ...
    test('should initialize successfully with valid environment', async () => {
        ...
        const mockHederaInstance = HederaService.mock.results[0].value;
    ...
    test('should handle agent registration when not already registered', async () => {
        agentRegistry.isAlreadyRegistered.mockResolvedValue(false);
        agentRegistry.registerAgent.mockResolvedValue({
            success: true,
            accountId: '0.0.999',
