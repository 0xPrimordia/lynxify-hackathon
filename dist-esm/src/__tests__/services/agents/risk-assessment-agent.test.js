...
class MockRiskAssessmentAgent {
    constructor(hederaService) {
        this.id = 'risk-assessment-agent';
        this.isRunning = false;
        this.priceHistory = new Map();
        this.highRiskThreshold = 0.1; // 10%
        this.mediumRiskThreshold = 0.05; // 5%
    ...
    async handlePriceUpdate(message) {
        ...
        if (!tokenId || typeof price !== 'number')
            return;
    ...
    async publishRiskAlert(tokenId, severity, priceChange, volatility) {
        const message = {
            id: `risk-${Date.now()}`,
            type: 'RiskAlert',
            timestamp: Date.now(),
...
describe('RiskAssessmentAgent', () => {
    ...
    describe('handlePriceUpdate', () => {
        ...
        it('should not publish alert for low risk changes', async () => {
            ...
            hederaService.publishHCSMessage.mockClear();
