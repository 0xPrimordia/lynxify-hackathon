...
describe('LynxifyAgent', () => {
    ...
    describe('Agent Construction', () => {
        ...
        test('should set up necessary event handlers during construction', () => {
            expect(mockEventBus.onEvent).toHaveBeenCalledWith(EventType.HCS10_REQUEST_RECEIVED, expect.any(Function));
            expect(mockEventBus.onEvent).toHaveBeenCalledWith(EventType.INDEX_REBALANCE_PROPOSED, expect.any(Function));
        });
    });
    describe('Agent Initialization', () => {
        test('should initialize all service components', async () => {
            ...
            expect(mockHederaService.subscribeToTopic).toHaveBeenCalledWith(testConfig.hcs10Config.registryTopicId, expect.any(Function));
            expect(mockHederaService.subscribeToTopic).toHaveBeenCalledWith(testConfig.hcs10Config.agentTopicId, expect.any(Function));
            expect(mockHederaService.subscribeToTopic).toHaveBeenCalledWith(testConfig.indexConfig.indexTopicId, expect.any(Function));
            // System initialized event should be emitted
            expect(mockEventBus.emitEvent).toHaveBeenCalledWith(EventType.SYSTEM_INITIALIZED, expect.anything());
        });
        test('should handle initialization errors from Hedera service', async () => {
            mockHederaService.initialize.mockRejectedValueOnce(new Error('Hedera service initialization error'));
            await expect(agent.initialize()).rejects.toThrow('Hedera service initialization error');
            expect(mockEventBus.emitEvent).toHaveBeenCalledWith(EventType.SYSTEM_ERROR, expect.any(Error));
        });
        test('should handle initialization errors from HCS10 service', async () => {
            mockHCS10Service.initialize.mockRejectedValueOnce(new Error('HCS10 service initialization error'));
        ...
        test('should handle initialization errors from Index service', async () => {
            mockIndexService.initialize.mockRejectedValueOnce(new Error('Index service initialization error'));
        ...
        test('should handle topic subscription errors', async () => {
            mockHederaService.subscribeToTopic.mockRejectedValueOnce(new Error('Topic subscription error'));
    ...
    describe('Agent Shutdown', () => {
        test('should shut down all service components', async () => {
            ...
            expect(mockEventBus.emitEvent).toHaveBeenCalledWith(EventType.SYSTEM_SHUTDOWN, undefined);
        });
        test('should handle shutdown errors gracefully', async () => {
            ...
            mockHederaService.shutdown.mockRejectedValueOnce(new Error('Shutdown error'));
