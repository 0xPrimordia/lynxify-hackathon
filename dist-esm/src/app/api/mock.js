...
export async function listenToMessages(topicId, callback) {
    const client = getClient();
    const query = new TopicMessageQuery()
        .setTopicId(TopicId.fromString(topicId));
    query.subscribe(client, (error) => {
...
export async function mockAgentAction(action, params) {
...
export async function updateTokenComposition(newWeights) {
...
export async function publishGovernanceSettings(topicId, message) {
    const client = getClient();
    const transaction = new TopicMessageSubmitTransaction()
        .setTopicId(TopicId.fromString(topicId))
        .setMessage(JSON.stringify(message));
