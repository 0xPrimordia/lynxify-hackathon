...
(function (ProposalType) {
    ProposalType["REBALANCE"] = "rebalance";
    ProposalType["PARAMETER_CHANGE"] = "parameter_change";
    ProposalType["TOKEN_ADDITION"] = "token_addition";
    ProposalType["TOKEN_REMOVAL"] = "token_removal";
    ProposalType["FEE_CHANGE"] = "fee_change";
...
export class GovernanceService {
     ...
     */
    constructor(hederaService, tokenService, config) {
     ...
     */
    handleProposalSubmission(message) {
        const { proposalId, title, description, type, creator, parameters } = message.details;
     ...
     */
    async validateProposalThreshold(proposal) {
        try {
    ...
    handleProposalVote(message) {
        ...
        this.validateVotingPower(voter, weight).then(validWeight => {
            ...
            if (existingVoteIndex >= 0) {
                ...
                proposal.votes[existingVote.voteType] -= existingVote.weight;
            ...
            proposal.votes[voteType] += validWeight;
     ...
     */
    async validateVotingPower(voter, claimedWeight) {
        try {
     ...
     */
    handleProposalExecution(message) {
     ...
     */
    async executeRebalanceProposal(proposal) {
        ...
        this.eventBus.emitEvent(EventType.INDEX_REBALANCE_PROPOSED, {
            ...
            newWeights,
            trigger: 'governance' // Force type for now, ideally update the trigger type in TokenizedIndexService
        });
     ...
     */
    async executeParameterChangeProposal(proposal) {
     ...
     */
    async executeTokenAdditionProposal(proposal) {
     ...
     */
    async executeTokenRemovalProposal(proposal) {
     ...
     */
    async executeFeeChangeProposal(proposal) {
     ...
     */
    async executeProtocolUpgradeProposal(proposal) {
     ...
     */
    handleProposalCancellation(message) {
     ...
     */
    async broadcastProposalStatus(proposal) {
        const statusMessage = {
            id: uuidv4(),
     ...
     */
    async createProposal(title, description, type, parameters, creator) {
     ...
     */
    async submitVote(proposalId, voter, voteType, weight) {
        const voteMessage = {
            id: uuidv4(),
            type: 'ProposalVote',
            timestamp: Date.now(),
     ...
     */
    async executeProposalById(proposalId, executor) {
     ...
     * Cancel a proposal
     */
    async cancelProposal(proposalId, canceler, reason) {
    ...
    async initialize() {
        ...
        try {
            ...
            this.proposalCheckInterval = setInterval(() => this.checkProposalExpirations(), 5 * 60 * 1000 // Every 5 minutes
            );
