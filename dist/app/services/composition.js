...
const hedera_1 = require("../utils/hedera");
async function calculateOptimalComposition(sectors, minLiquidityThreshold = 0.1 // 10% liquidity to market cap ratio as default
) {
    const composition = [];
    for (const sector of sectors) {
        ...
        for (const token of sector.tokens) {
            if (!token.tokenId)
                continue;
            const liquidityData = await (0, hedera_1.getTokenLiquidity)(token.tokenId);
            if (!liquidityData)
                continue;
        ...
        for (const weight of weights) {
            const tokenWeight = (0, hedera_1.calculateTokenRatio)(sectorMarketCap / weights.length, // Simplified market cap distribution
            {
...
function isRebalancingNeeded(currentWeights, targetWeights, driftThreshold = 5 // 5% drift threshold by default
) {
