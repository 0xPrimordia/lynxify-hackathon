...
async function registerHCS10Agent() {
  try {
    console.log('🚀 Initializing HCS-10 agent registration from scratch...');
    ...
    // First validate credentials with Hedera SDK directly
    try {
      ...
      // Check if we can get account info - this validates credentials work
      const balance = await client.getAccountBalance(AccountId.fromString(operatorId));
      console.log(`✅ Credentials valid! Account balance: ${balance.hbars.toString()}`);
    } catch (error) {
      console.error('❌ Failed to validate Hedera credentials:', error);
      throw new Error('Invalid Hedera credentials. Please check your account ID and private key.');
    }
    ...
    // Initialize the HCS10 client with exact format required by the SDK
    ...
    // Register the agent using the SDK
    // This creates all required topics and registers with the Moonscape registry
    const agentInfo = await hcs10Client.registerAgent({
      displayName: "Fresh HCS-10 Agent",
      description: "A brand new agent created from scratch",
      memo: "HCS-10 Test Agent - Created " + new Date().toISOString()
    });
    ...
    if (error.message.includes('parse entity id')) {
      ...
      console.error('   and Hedera SDK versions. Try installing versions that are known to work:');
