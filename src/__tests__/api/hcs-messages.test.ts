...
describe('HCS Messages API Route', () => {
  ...
  test('should propose a rebalance successfully', async () => {
    ...
    expect(hederaModule.hederaService.proposeRebalance).toHaveBeenCalledWith(
      { 'BTC': 0.5, 'ETH': 0.3, 'SOL': 0.2 },
      expect.any(Number),
      0.51,
      'market_volatility',
      'High market volatility requires rebalancing'
    );
  });
  ...
  test('should handle executing a rebalance', async () => {
    ...
    expect(hederaModule.hederaService.executeRebalance).toHaveBeenCalledWith(
      'P1234567890',
      { 'BTC': 0.6, 'ETH': 0.3, 'SOL': 0.1 }
    );
  });
