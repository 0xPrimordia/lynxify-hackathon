...
jest.mock('../../app/services/token-service', () => {
  ...
  return {
    TokenService: jest.fn().mockImplementation(() => ({
      ...
      getBalance: jest.fn().mockImplementation((tokenId: string) => {
...
describe('Token Rebalance Integration Flow', () => {
  ...
  test('Price-Based Rebalance Flow with Token Operations', async () => {
    ...
    const proposalEvents = (eventBus.emitEvent as jest.Mock).mock.calls.filter(
      call => call[0] === EventType.INDEX_REBALANCE_PROPOSED
    );
  ...
  test('Multiple Sequential Rebalances', async () => {
    ...
    const proposalEvents1 = (eventBus.emitEvent as jest.Mock).mock.calls.filter(
      call => call[0] === EventType.INDEX_REBALANCE_PROPOSED
    );
    ...
    const proposalEvents2 = (eventBus.emitEvent as jest.Mock).mock.calls.filter(
      call => call[0] === EventType.INDEX_REBALANCE_PROPOSED
    );
