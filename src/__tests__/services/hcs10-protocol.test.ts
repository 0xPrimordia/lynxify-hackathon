import { HCS10ProtocolService, HCS10ProtocolConfig, RegistrationStatus } from '../../app/services/hcs10-protocol';
import { SharedHederaService } from '../../app/services/shared-hedera-service';
import { EventBus, EventType } from '../../app/utils/event-emitter';
import { isRebalanceProposal, isRebalanceApproved, TokenWeights } from '../../app/types/hcs';
...
describe('HCS10ProtocolService', () => {
  let service: HCS10ProtocolService;
  let mockHederaService: SharedHederaService;
  let defaultConfig: HCS10ProtocolConfig;
  ...
  describe('Message Type Validation', () => {
    test('should correctly identify rebalance proposal messages', () => {
      ...
      const nonProposalMessage1: HCSMessage = {
        id: 'test-id-2',
        type: 'PriceUpdate',
        timestamp: Date.now(),
        sender: 'test-sender',
      ...
      const nonProposalMessage2: HCSMessage = {
        id: 'test-id-3',
        type: 'RebalanceProposal',
        timestamp: Date.now(),
        sender: 'test-sender',
    ...
    test('should correctly identify rebalance approval messages', () => {
      ...
      // Negative test case - wrong type
      const nonApprovalMessage1: HCSMessage = {
        id: 'test-id-5',
        type: 'RebalanceProposal',
        timestamp: Date.now(),
        sender: 'test-sender',
    ...
    test('should validate token weights format', () => {
      // Valid weights
      const validWeights: TokenWeights = {
        'BTC': 0.4,
        'ETH': 0.4,
        'SOL': 0.2
      };
      ...
      const invalidWeights1: TokenWeights = {
        'BTC': 0.5,
        'ETH': 0.4,
        'SOL': 0.2
      };
      ...
      const invalidWeights2: TokenWeights = {
        'BTC': 0.4,
      ...
      const validProposal: HCSMessage = {
        id: 'test-id-6',
        type: 'RebalanceProposal',
        timestamp: Date.now(),
        sender: 'test-sender',
      ...
      const invalidProposal1: HCSMessage = {
        id: 'test-id-7',
        type: 'RebalanceProposal',
        timestamp: Date.now(),
        sender: 'test-sender',
      ...
      const invalidProposal2: HCSMessage = {
        id: 'test-id-8',
        type: 'RebalanceProposal',
        timestamp: Date.now(),
        sender: 'test-sender',
    ...
    test('should validate HCS message format correctly', () => {
      ...
      expect(service['isValidHCS10Message'](invalidMessage1 as any)).toBe(false);
      expect(service['isValidHCS10Message'](invalidMessage2 as any)).toBe(false); // Type isn't valid
      
      // These may return undefined or other values rather than false
      // so just check they don't throw errors
      expect(() => service['isValidHCS10Message'](messageWithoutContents as any)).not.toThrow();
