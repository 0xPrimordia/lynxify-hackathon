...
export class ConnectionHandlerService {
  ...
  async startListening(): Promise<boolean> {
    ...
    try {
      ...
      setInterval(async () => {
        if (!this.isListening || !this.inboundTopicId) return;
   ...
   */
  private async handleConnectionRequest(message: any, sequenceNumber: number): Promise<void> {
    try {
...
export function createConnectionHandler(client: HCS10Client): ConnectionHandlerService {
