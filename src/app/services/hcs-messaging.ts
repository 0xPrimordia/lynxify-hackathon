...
// Define HCS-10 formatted message type
export type HCS10Message = {
  p: string;           // Protocol identifier
  op: string;          // Operation type
  operator_id?: string; // Operator ID in format "topicId@accountId"
  account_id?: string;  // Used for register operation
...
export class HCSMessagingService {
  ...
  async sendToMoonscape(
    ...
    try {
      ...
      const moonscapeMessage: HCS10Message = {
      ...
      await this.hederaService.publishHCSMessage(outboundTopicId, moonscapeMessage as any);
  ...
  async sendAgentStatus(
    ...
    try {
      ...
      const statusMessage: HCS10Message = {
        p: "hcs-10",
        op: "message",
        operator_id: `${inboundTopicId}@${operatorId}`,
        data: JSON.stringify({
      ...
      await this.hederaService.publishHCSMessage(outboundTopicId, statusMessage as any);
   ...
   */
  async sendHCSMessage(topicId: string, message: HCSMessage): Promise<boolean> {
    if (!topicId) {
