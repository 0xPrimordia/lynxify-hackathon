...
export class MessageProcessorService {
  ...
  private messageHandlers: Map<string, (message: any, senderId: string) => Promise<void>> = new Map();
   ...
   */
  private async monitorConnectionTopic(requesterId: string, topicId: string): Promise<void> {
    try {
   ...
   */
  private getMessageContent(message: HCSMessage): string | null {
   ...
   */
  private async processMessage(message: HCSMessage, senderId: string): Promise<void> {
    try {
   ...
   * @param senderId The sender's account ID
   */
  private async processStandardMessage(message: any, senderId: string): Promise<void> {
    try {
   ...
   * @param senderId The sender's account ID
   */
  private async processHCS1Reference(reference: string, senderId: string): Promise<void> {
    try {
   ...
   * @param senderId The sender's account ID
   */
  private async processCloseConnection(message: any, senderId: string): Promise<void> {
    try {
   ...
   */
  async sendMessage(requesterId: string, message: any): Promise<boolean> {
    try {
   ...
   * @param message The message to send
   */
  async sendLargeMessage(requesterId: string, message: any): Promise<boolean> {
    try {
      // Get the connection topic for this requester
      const connectionTopicId = this.connections.get(requesterId);
      if (!connectionTopicId) {
   ...
   * @private
   */
  private async createStorageTopic(): Promise<string> {
    try {
...
export function createMessageProcessor(client: HCS10Client): MessageProcessorService {
