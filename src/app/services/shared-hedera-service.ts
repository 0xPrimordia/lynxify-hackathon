...
/**
 * Extended TopicMessage interface for SDK compatibility
 * This creates a superset of possible TopicMessage properties across SDK versions
 */
interface ExtendedTopicMessage {
  sequenceNumber?: {
    toNumber: () => number;
  };
  consensusTimestamp?: {
  ...
  contents?: Uint8Array;
  content?: Uint8Array;
  // Add other potential TopicMessage properties here
}
...
export class SharedHederaService extends EventEmitter {
  ...
  private topicCallbacks: Map<string, Set<(message: HCSTopicMessage) => void>> = new Map();
   ...
   */
  private initializeClient(config?: SharedHederaConfig): Client {
   ...
   */
  public async verifyTopics(): Promise<void> {
   ...
   */
  public async sendMessage(topicId: string, message: string | object): Promise<{
  ...
  public async subscribeToTopic(
    topicId: string,
    callback: (message: HCSTopicMessage) => void
  ): Promise<void> {
      ...
      const subscription = topicMessageQuery.subscribe(
        ...
        (message) => {
          ...
          try {
            ...
            const extMessage = message as unknown as ExtendedTopicMessage;
            ...
            const topicMessage: HCSTopicMessage = {
              id: uuidv4(),
              topicId: topicId,
   ...
   */
  public async unsubscribeFromTopic(topicId: string): Promise<void> {
   ...
   */
  public formatTopicId(topicId: string): string {
   ...
   */
  public getTopicStatus(): Record<string, string | boolean | string[]> {
    const status: Record<string, string | boolean | string[]> = {};
