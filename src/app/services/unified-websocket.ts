...
export class UnifiedWebSocketService {
  ...
  private httpServer: Server | null = null; // Store the HTTP server reference
  private externalServer: boolean = false; // Track if using an external server
  private clients: Set<WebSocket> = new Set();
  private eventBus: EventBus;
  private lynxifyAgent: LynxifyAgent;
  private tokenService: TokenService;
  private indexService: TokenizedIndexService;
  ...
  constructor(
    ...
    indexService: TokenizedIndexService,
    port: number = 3001,
   ...
   */
  private handleTokenOperation(ws: WebSocket, data: any): void {
    ...
    switch (operation) {
      case 'get_balances':
        this.tokenService.getTokenBalances()
          .then((balances: Record<string, number>) => {
            this.sendToClient(ws, {
              type: 'token_balances',
              data: balances
            });
   ...
   */
  private handleRebalanceProposal(ws: WebSocket, data: any): void {
    const { weights } = data;
   ...
   */
  private async sendAgentStatus(ws: WebSocket): Promise<void> {
    try {
      ...
      this.sendToClient(ws, {
        type: 'error',
        data: { message: 'Failed to get agent status: ' + (error as Error).message }
      });
    }
  }
