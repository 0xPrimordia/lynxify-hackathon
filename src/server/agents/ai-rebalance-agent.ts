...
export class AIRebalanceAgent {
  private openai: OpenAI;
  private supportedTokens: TokenInfo[];
  private lastRebalanceTime: number = 0;
  private rebalanceIntervalHours: number = 24; // Default to daily rebalancing
  ...
  private treasuryState: Treasury | null = null;
  ...
  public async updateMarketData(data: Partial<MarketData>): Promise<void> {
  ...
  public async updateTreasuryState(treasury: Treasury): Promise<void> {
  ...
  public async generateRebalanceProposal(): Promise<RebalanceResult> {
    const now = Date.now();
  ...
  private async generatePortfolioAllocation(): Promise<TokenWeights> {
    ...
    try {
      ...
      const newWeights = JSON.parse(content) as TokenWeights;
      ...
      const defaultWeights: TokenWeights = {};
      this.supportedTokens.forEach(token => {
  ...
  private async analyzeProposedChanges(newWeights: TokenWeights): Promise<string> {
    ...
    ${this.supportedTokens.map(token => 
      `${token.symbol} (${token.name}): ${(token.currentWeight * 100).toFixed(2)}%`
    ).join('\n')}
    
    NEW ALLOCATION:
    ${Object.entries(newWeights).map(([symbol, weight]) => 
  ...
  private async submitRebalanceProposal(newWeights: TokenWeights, analysis: string): Promise<void> {
    ...
    await hederaService.proposeRebalance(
      ...
      executeAfter,
      quorum,
      'scheduled', // Using a valid trigger type from the allowed options
      analysis
    );
  ...
  public async checkAndGenerateRebalance(): Promise<boolean> {
...
export const aiRebalanceAgent = new AIRebalanceAgent(); 