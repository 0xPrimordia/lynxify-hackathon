...
// Extend the HCSMessage interface to include our test field
interface TestHCSMessage extends HCSMessage {
  details: HCSMessage['details'] & {
    testId?: string;
  };
}
...
async function runTest() {
  try {
    ...
    const messagePromise = new Promise<void>((resolve) => {
      openConvAIService.subscribeToTopic(testTopic, (message: HCSMessage) => {
        console.log('📩 Received message:', JSON.stringify(message, null, 2));
        
        // Convert to our extended test interface to check for testId
        const testMessage = message as TestHCSMessage;
    ...
    await openConvAIService.sendMessage(testTopic, testMessage);
